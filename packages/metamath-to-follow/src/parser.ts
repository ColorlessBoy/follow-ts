...
export class FrameStack {
  ...
  public addFloat(keyword: Token, statement: Array<Token>, label?: Token) {
    ...
    if (floatNode.error === undefined) {
      const varname = floatNode.variable?.value;
      if (varname) {
        frame.floatVarMap.set(varname, floatNode);
      }
  ...
  public addEssential(keyword: Token, statement: Array<Token>, label?: Token) {
    ...
    for (; idx < statement.length; ++idx) {
      ...
      if (this.lookUpConst(token)) {
        ...
        essentialNode.bodyAbstract.push(token);
      } else if (this.lookUpVar(token)) {
        token.tokenType = TokenType.VAR_VAL;
        essentialNode.body.push(token);
        const floats = this.lookUpFloatByVar(token.value);
        if (floats?.type) {
          essentialNode.bodyAbstract.push(floats.type);
        }
      } else {
  ...
  public addAxiom(keyword: Token, statement: Array<Token>, label?: Token) {
    ...
    for (; idx < statement.length; ++idx) {
      ...
      if (this.lookUpConst(token)) {
        ...
        axiomNode.bodyAbstract.push(token);
      } else if (this.lookUpVar(token)) {
        token.tokenType = TokenType.VAR_VAL;
        axiomNode.body.push(token);
        const float = this.lookUpFloatByVar(token.value);
        if (float?.type) {
          axiomNode.bodyAbstract.push(float.type);
        }
      } else {
  ...
  public addProve(keyword: Token, statement: Array<Token>, proof: Array<Token>, label?: Token) {
    ...
    for (; idx < statement.length; ++idx) {
      ...
      if (this.lookUpConst(token)) {
        ...
        proveNode.bodyAbstract.push(token);
      } else if (this.lookUpVar(token)) {
        token.tokenType = TokenType.VAR_VAL;
        proveNode.body.push(token);
        const float = this.lookUpFloatByVar(token.value);
        if (float?.type) {
          proveNode.bodyAbstract.push(float.type);
        }
      } else {
  ...
  public lookUpFloatByVar(varname: string): FloatNode | undefined {
    for (let i = this.stack.length - 1; i >= 0; i--) {
      const frame = this.stack[i];
      const floatDefToken = frame.floatVarMap.get(varname);
      if (floatDefToken) {
        return floatDefToken;
      }
    }
  ...
  private makeAssertion(node: AxiomNode | ProveNode) {
      ...
      for (let j = frame.floats.length - 1; j >= 0; j--) {
        ...
        if (floatName && varSet.has(floatName)) {
          ...
          if (float.label?.value) {
            node.floatMap.set(float.label.value, float);
          }
          if (float.variable?.value) {
            node.floatVarMap.set(float.variable?.value, float);
          }
    ...
    node.floats.reverse();
    if (node.floats.length > 1) {
      const nextFloats: Array<string> = [];
      const floatVarLog: Set<string> = new Set();
      for (const essential of node.essentials) {
        for (const token of essential.body) {
          const float = node.floatVarMap.get(token.value);
          if (float && float.label?.value && !floatVarLog.has(token.value)) {
            nextFloats.push(float.label.value);
            floatVarLog.add(token.value);
          }
        }
      }
      for (const token of node.body) {
      ...
      const strOrderMap: Map<string, number> = new Map();
      nextFloats.forEach((float, idx) => {
        if (float) {
          strOrderMap.set(float, idx);
        }
      });
      node.floatsStrOrder = node.floats.map((float) => {
        if (float.label?.value) {
          return strOrderMap.get(float.label.value) || 0;
        }
        return 0;
      });
    }
  ...
  private verifyProof(proveNode: ProveNode) {
    ...
    for (const node of decompressProof) {
      ...
      if (opNode.body) {
        for (const op of opNode.body) {
          if (op.tokenType === TokenType.VAR_VAL) {
            opNode.varSet.add(op.value);
          }
        }
      }
    }

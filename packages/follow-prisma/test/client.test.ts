...
test('#4 toJson', async () => {
  ...
  const titleList = client.generateFileIndex(markdownBlock);
  const blockTitleIndex = await client.generateBlockFileIndexMap(markdownBlock);
  ...
  const indexJson = {
    ...
    blockTitleIndex: blockTitleIndex.map((e) => {
      return [e.name, e.target, e.index];
    }),
  };
